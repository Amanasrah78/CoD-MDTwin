...
SESSION_DIR = ROOT / "results" / "metrics" / "chain_depth_sessions"
COMBINED_RAW = ROOT / "results" / "metrics" / "chain_depth_sensitivity_multisession_raw.csv"
SUMMARY_CSV = ROOT / "results" / "metrics" / "chain_depth_sensitivity_multisession_summary.csv"
RESULTS_JSON = ROOT / "results" / "metrics" / "chain_depth_sensitivity_multisession_results.json"
...
def main() -> None:
    ...
    for path in raw_paths:
        ...
        for source in session_rows:
            ...
            row: Dict[str, Any] = {
                ...
                "chain_credentials": int(source["chain_credentials"]),
                "chain_size_bytes": int(source["chain_size_bytes"]),
            }
            rows.append(row)
    ...
    for depth in EXPECTED_DEPTHS:
        ...
        sizes = [int(row["chain_size_bytes"]) for row in accepted_rows]
        ...
        summaries.append(
            {
                ...
                "chain_size_bytes_mean": rounded(statistics.fmean(sizes)),
            }
        )
    ...
    report = {
        "test": "chain_depth_sensitivity_multisession",
        "methodology": {
            ...
            "mean_and_percentiles": "pooled accepted-operation latencies",
